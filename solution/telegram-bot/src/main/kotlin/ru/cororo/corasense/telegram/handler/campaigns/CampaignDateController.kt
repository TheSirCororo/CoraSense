...
import ru.cororo.corasense.shared.service.hasStarted
...
suspend fun changeDate(id: String, user: User, bot: TelegramBot, update: CallbackQueryUpdate) = telegramApi {
    ...
    message { "Введите новую дату ${if (changingStartDate) "начала" else "конца"} показа для кампании." }.inlineKeyboardMarkup { cancelButton() }
        .send(user, bot)
