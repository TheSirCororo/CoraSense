...
class LlmServiceImpl(private val application: Application, client: HttpClient? = null) : CoroutineScope, LlmService {
    ...
    override suspend fun isActive() = llmEnabled

    private suspend fun prompt(text: String) = this@LlmServiceImpl.client.post(groqBaseUrl) {
    ...
    override suspend fun moderateText(text: String) =
        prompt("$MODERATION_TEXT_PROMPT\nФраза: $text").let {
            Json.decodeFromString<ModerationVerdict>(it)
        }

    override suspend fun generateCampaignText(campaignTitle: String, advertiserName: String) =
